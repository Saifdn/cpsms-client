...
import { frameShipmentColumns } from "@/components/columns/FrameShipmentColumns";
import { frameSubmittedColumns } from "@/components/columns/FrameSubmittedColumns";

import {
  usePendingFrameOrderShipments,
  useSubmittedFrameOrderShipments,
  useGetFrameOrderQuotation,
  useBulkProcessFrameOrderShipments,
  useDownloadFrameOrderMergedAwb,
...
const FrameShipmentManagement = () => {
  ...
  } = usePendingFrameOrderShipments({ date: dateParam, page: currentPage, limit });

  const {
    data: submittedData,
    isLoading: submittedLoading,
    refetch: refetchSubmitted,
  } = useSubmittedFrameOrderShipments({ date: dateParam, page: currentPage, limit });

  const getQuotationMutation = useGetFrameOrderQuotation();
  const bulkProcessMutation = useBulkProcessFrameOrderShipments();
  const downloadMergedAwbMutation = useDownloadFrameOrderMergedAwb();
  ...
  return (
    <Page>
      <PageHeader
        title="Frame Shipments"
        description="Get quotation and ship ala-carte frame orders in bulk"
      />
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center mb-6">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-sm text-muted-foreground">
            {isPending ? (
              <>
                {pagination?.total ?? pendingShipments.length} frame orders ready •{" "}
              ...
              <>
                {pagination?.total ?? submittedShipments.length} frame orders submitted •{" "}
                <span className="font-medium text-foreground">
      ...
      {isPending && (
        <DataTable
          title="Pending Frame Shipments"
          description="Select frame orders and click Get Quotation"
          columns={frameShipmentColumns}
      ...
      {!isPending && (
        <DataTable
          title="Submitted Frame Shipments"
          description="Frame orders that have been submitted for shipping"
          columns={frameSubmittedColumns}
...
export default FrameShipmentManagement;
